...
canvas.addEventListener('mousemove', function (e) {
    ...
    document.getElementById('y-value-move').textContent = (e.y);

    var normalizedx = parseFloat((((((e.x)/(canvas.width + 1))*2) - 1)).toFixed(2))
    var normalizedy = parseFloat((((((e.y)/(canvas.height + 1))*2) - 1)).toFixed(2))

    document.getElementById('x-value-move-normalized').textContent = normalizedx;
    document.getElementById('y-value-move-normalized').textContent = normalizedy * -1;
})
